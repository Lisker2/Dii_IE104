...
public class TspInstance {
	...
	public TspInstance(String name) {
		//System.out.println(this.instanceNames.length);
		String instance_path = name + ".tsp";
		try{
			FileReader fr = new FileReader(instance_path);
			BufferedReader bfr = new BufferedReader(fr);
			loadData(bfr);
		}catch(Exception ex1){
			try{
			BufferedReader bfr = new BufferedReader(new InputStreamReader(this
					.getClass().getClassLoader().getResourceAsStream(instance_path)));
			loadData(bfr);
			}catch(Exception ex2){
				ex1.printStackTrace();
				ex2.printStackTrace();
				System.err.print("problem when opening file "+instance_path);
			}
		}
		
		this.loadNearestCities(name);
	}
	
	public void loadNearestCities(String name) {

		
		String fileName = name +"NearestCities.txt";
		try{
			...
			try{
			fileName = name +"NearestCities.txt";
			BufferedReader bfr = new BufferedReader(new InputStreamReader(this
	...
	private void loadData(BufferedReader bfr) throws Exception{
		...
		System.out.println(this.numbCities);
